...
const formatDuration = (duration, isBold) => {
  const color = isBold ? pc.bold : (x) => x;
  if (duration < 1000) {
...
const formatTimes = (event) => {
  ...
  if (event.duration !== range)
    additionalInfo.push(`self ${formatDuration(event.duration, pc.bold)}`);
...
const formatEvent = (event) => {
  ...
  switch (event.name) {
    case "webpack-compilation":
      head = `${pc.bold(`${event.tags.name} compilation`)} ${formatTimes(
        event
      )}`;
      break;
    ...
    default:
      if (event.name.startsWith("build-module-")) {
        ...
        head = `${pc.magenta("module")} ${
          packageName
            ? `${pc.bold(pc.cyan(packageName))} (${formatFilename(
                event.tags.name
