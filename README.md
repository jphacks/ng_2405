...
## 製品概要
### 背景(製品開発のきっかけ、課題等）
IT学習（プログラミング学習）において「とりあえず技術を学んだがそれをどのように使うのかが分からない」，「教科書や本の内容だけでなくより実用的な開発をしたい」という段階が必ずある．
そういったときにプログラミングのタスクを提供してくれるアプリはあるが，ユーザーの学習状況にあったタスクををアプリ側が提示してくれるようなサービスはあまりない．
そのため，そういったタスクを生成AIに作成させようという発想に至った．
### 製品説明（具体的な製品の説明）
使う言語，使う技術をユーザーが指定すると生成AIが適切な課題目標を提示してくれる
### 特長
...
### 解決出来ること
「技術を学んだが定着しているかが分からない」といったときにその技術に則したタスクを解くことができるかどうかで技術の定着度合いが分かる．
### 今後の展望
- ユーザーが詰まった時などにヒントを見られる仕組み
- 達成したタスク数に応じてユーザーが恩恵を受けられる仕組み
### 注力したこと（こだわり等）
...
- /back/app/main.py get_task_from_gemini
# JPHacks
## テーマ
